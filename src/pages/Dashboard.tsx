...
import taskReducer, { saveLocal, taskInit } from '../typedReducers/taskReducer';

function Dashboard() {
  const [Tasks, taskDispatch] = useReducer(taskReducer, [], taskInit);
  const [Metric, metricDispatch] = useReducer(metricReducer, [], () =>
    metricInit([])
  );
  ...
  const [isToday, isThisWeek, isThisMonth, isThisYear] = getDateStatus(
    Metric[Metric.length - 1]?.date || ''
  );

  const progressions = {
    Today: [getTodayCompletedMetricCount(), getTodayActiveMetricCount()],
    'This Week': [
      getThisWeekCompletedMetricCount(),
      getThisWeekActiveMetricCount(),
    ],
    'This Month': [0, 0],
    'All Time': [0, 0],
  };
  ...
  return (
    <div className="mx-6 grid h-[calc(screen-48px-6rem)] w-[calc(screen-48px)] grid-cols-6 gap-6 p-6">
      <div className="col-span-1 grid grid-rows-4 gap-6">
        {Object.entries(progressions).map(
          ([title, [numerator, denominator]]) => (
            <section
              key={title}
              className="bg-dracula-comment col-start-1 h-fit w-full max-w-60 min-w-40 rounded-2xl p-4 text-center text-4xl"
            >
              <svg
                viewBox={`0 0 100 35`}
